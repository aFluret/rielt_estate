...
public class Validator {
    ...
    private static final String REGEX_FOR_PASSWORD = "\\w{5,}";
    //private static final String REGEX_FOR_PASSWORD = "^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z])(?!.*\\s).*$";
    private static final String REGEX_FOR_ACCOUNT_NUMBER = "^[0-9]{16}$";
    ...
    private static final String REGEX_FOR_SERVICE_NAME = "([A-Z][a-z\\s\\-\\`A-Z]+)|([А-Я][а-я\\s\\-\\`А-Я]+)";
    ...
    public final static boolean matchServiceQuantity(Integer... integers) {
    ...
    public final static boolean matchServiceWeightCost(Double... doubles) {
    ...
    public final static boolean matchServiceName(String... strings) {
        Pattern pattern = Pattern.compile(REGEX_FOR_SERVICE_NAME);
        Matcher matcher;
        for (String s : strings) {
            matcher = pattern.matcher(s);
            if (!matcher.matches()) {
